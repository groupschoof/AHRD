...
public class Parameters implements Cloneable, Comparable<Parameters> {
	...
	private static final int numberOfDataBaseIndependantParameters = 6;
	 ...
	 * average training-score (objective-function).
	 */
	 ...
	 * if, the last mutation resulted in an increase of the average training
	 * score.
	 */
	 ...
	 * else
	 * 
	 * @param increaseInAvgTrainingScore
	 *            - The difference in average training scores between the
	 *            currently accepted parameters and the currently evaluated
	 *            ones.
	 * @return Double
	 */
	public static Double pMutateSameParameter(
			Double increaseInAvgTrainingScore) {
		double p = 0.0;
		if (increaseInAvgTrainingScore != null
				&& increaseInAvgTrainingScore > 0.0) {
			Double s = getSettings().getpMutateSameParameterScale();
			p = (Math.exp(-(1.0 - increaseInAvgTrainingScore)) + s)
					/ (Math.exp(0) + s);
		}
		return p;
	}
	 ...
	 * <li>GoTermScoreInformationContentWeight</li>
	 * <li>DescriptionScoreThreshold</li>
	...
	public Parameters neighbour(Double diffTrainingScoreToLastEvaluatedParams) {
		...
		if (randParamToMutate < numberOfDataBaseIndependantParameters) {
			...
			else if (randParamToMutate == 4)
				ngb.mutateGoTermScoreInformationContentWeight();
			else if (randParamToMutate == 5) {
		...
		// Reset average training score
		ngb.setAvgTrainingScore(null);
	...
	public Parameters recombine(Parameters partner) {
		...
		offspring.setAvgTrainingScore(null);
	...
	public void setAvgTrainingScore(Double avgTrainingScore) {
	...
	/**
	* Compares the average training score of the current Parameters with 
	* the average training score of the specified parameters for order. 
	* Returns a negative integer, zero, or a positive integer as the average 
	* training score of the current Parameters of these Parameters is less
    * than, equal to, or greater than the average training Score of the 
    ...
    * */
	...
	public int compareTo(Parameters other) {
		if (this.getAvgTrainingScore() != null && other.getAvgTrainingScore() != null){
			if (this.getAvgTrainingScore() < other.getAvgTrainingScore())
				return -1;
			if (this.getAvgTrainingScore() > other.getAvgTrainingScore())
				return 1;
