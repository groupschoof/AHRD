...
In addition to description annotations, Gene Ontology term annotations can be evaluated as well. 
...
Basically the input file is almost identical to the standard AHRD run. The following additional parameters are available to modify the behavior of _both_ approaches; all numeric values given show the _default_ used for the respective parameter:
...
# Set the regular expression to extract short protein accessions mapped to GO terms from the database GOA using @gene_ontology_reference_regex@, e.g. @gene_ontology_reference_regex: ^UniProtKB\\s+(?<shortAccession>\\S+)\\s+\\S+\\s+(?<goTerm>GO:\\d{7})@
# Set the Blast database specific regular expression, used to extract the short accessions from long ones with @short_accession_regex:@ For example for TAIR10, use @short_accession_regex: "^(?<shortAccession>[^\\.]+)(\\.\\d+)?$"@.
