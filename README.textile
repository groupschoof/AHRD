...
Copy (clone) AHRD to your computer using git via commandline:
...
h5. 2.2.3.1 Test custom blascklists and filters
...
Shows how the first regular expression does not match anything in the test string "activity", but after matching it agains the second regular expression nothing remains. The test applies all provided regular expression _in order of appeareance_ and shows what _remains_ of the provided test string after each iteration.
...
_Batch-Name requirement:_ All above explained files belonging to the same Batch _must_ have the same name. This name must start with alpha-numeric characters and may finish with digits indicating the Batche's number. File extensions are allowed to be varying. 
