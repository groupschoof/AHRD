...
databases that do not necessarily have to be the above ones. If annotating
genes from a fungal genome searching for yeast-sequences of high similarity
might be more recommendable.

All parameters can be set manually, or the default ones can be used as given in
the example input file ./test/resources/ahrd_input.yml (see Section
Parameters).
