...
git checkout tags/2.0-stable</pre>
...
Optionally domain search results from InterProScan and functional annotations
from Gene Ontology can be given as additional input to improve the results.
...
Shows how the first regular expression does not match anything in the test string "activity", but after matching it against the second regular expression nothing remains. The test applies all provided regular expression _in order of appearance_ and shows what _remains_ of the provided test string after each iteration.
...
<img src="http://github.com/groupschoof/AHRD/raw/master/images/formulae.jpg" />
...
h4. 3.3.2 The weight in formula Lexical-Score is

<pre>description_score_relative_description_frequency_weight: 0.6 </pre>

h4. 3.3.3 The weight in formula Description-Score also is Blast-database specific
...
h3. 4.1 Parse Blast-XML output

The code is almost finished. What is needed is to get the already available data copied into our custom BlastResult objects and have a proper test method for this creation of BlastResult instances from the new JAXB Parser. Classes are in package @ahrd.model.blast@ and "full parser" in @ahrd.test.BlastXmlOutputParser.java@. See @build.xml@ section "test.parse.blastout.xml" for details or test parser *even on very large blast xml output files* using the command line. Also do not forget to implement a decision switch to use BlastXmlParser if and only if the blast output file terminates in @.xml@.
