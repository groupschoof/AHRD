...
All parameters can be set manually, or the default ones can be used as given in the example input file ./test/resources/ahrd_input_test_run.yml (see section Parameters).

In order to parallelize the protein function annotation processes,  AHRD can be run on batches of recommended size between 1,000 to 2,000 proteins.  If you want to annotate very large protein sets or have low memory capacities use the included Batcher to split your input-data into Batches of appropriate size (see section Batcher). _Note:_ As of Java 7 or higher AHRD is quite fast and batching might no longer be necessary.
...
_Note_: For details on extended GO annotation tables please refer to section 3.3.2.2 
...
_Note_, that the Trainer-Batcher works very much like the AHRD-Batcher (section 2.3). Particularly you need to respect the _batch-name requirements_ explained there.
