...
If you have well annotated reference proteins from an organism closely realted to the proteins you want to annotate, you can optimize AHRD's parameters to reproduce protein function predictions more alike to your references. In order to do so you need to start a simulated annealing approach:
...
Basically the input file is almost identical to the standard AHRD run. Only the following additional parameters are available:

| Parameter | required? | Description |
| @path_log: your_log_file.tsv@ | no | Stores the parameters evaluated in any optimization iteration, their F1-Score, and the difference in score to the currently accepted parameter set. |
