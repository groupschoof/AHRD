...
h3. 2.2.2 Running AHRD considering domain architecture similarities:

AHRD can take domain architecture of similar proteins into account to assign
human readable descriptions of candidates not also of high sequence similarity
and frequently appearing words in their descriptions but especially having a
domain architecture like the one of the query protein. 

To do so, provide InterPro-Scan results for the query proteins with input parameter:
<pre>interpro_result: query_proteins_iprscn.tsv</pre>

Also provide the following additional weights:
<pre>domain_weights_database: ./test/resources/domain_weights_database.txt
description_score_domain_similarity_weight: 0.2 
token_score_domain_similarity_weight: 0.2</pre>

_Important note:_ Remember to adjust all token_score_x_weights to sum up to 1.0!

AHRD assigns domain architecture similarity scores based on the formulae as
described in this article: 

bq. Lee, Byungwook, and Doheon Lee. "Protein Comparison at the Domain Architecture Level." 
BMC Bioinformatics 10, no. Suppl 15 (December 3, 2009): S5.

The following parameter decides how AHRD should
compute domain architecture similarity. Based on Pfam or InterPro
annotations:@compute_domain_similarity_on: pfam@ If you want o use
either domain annotation please remember to provide the respective _domain
weights database_.

In order to retrieve Pfam or InterPro annotations for the description
candidates (Blast results) AHRD queries the RESTful UniprotKB web service.
Hence, running AHRD in this mode, you require Internet access. This is done in
parallel in order to speed this process up, but still it significantly
increases AHRD's run time by 5 to 15 seconds per query protein approximately. 

h3. 2.2.2.1 Example AHRD input. Template for AHRD run including domain architecture similarity scoring.

Use file @./test/resources/ahrd_dom_arch_sim_input.yml@ 
...
AHRD supports three different formats. The default one is a tab-delimited table.
Others are FASTA-, and XML-Format.
...
To set AHRD to write its output in XML-Format set the following switch in the input.yml:

<pre>
output_format: 'xml'
</pre>

AHRD will write a valid XML-File.
