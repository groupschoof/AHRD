...
h4. 3.3.3 Parameters controlling the parsing of tabular sequence similarity search result tables (legacy BLAST, BLAST+, and BLAT)
...
h4. 3.3.4 Parameters controlling Gene Ontology term annotations
...
h5. 3.4.4.1 Custom reference Gene Ontology annotations (non UniprotKB GOA)
...
h5. 3.4.4.2 Have AHRD extend predicted GO terms with ancestor GO terms
...
h6. 3.4.4.2.1 Custom Gene Ontology Database

As mentioned above (3.4.4.2) AHRD needs an instance of the Gene Ontology MySQL database, if you want it to generate an extended GO annotation table. Instead of using the default public database provided by the EBI, you can set the connection parameters to your own database:
