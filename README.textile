...
# Set the regular expression to extract short protein accessions mapped to GO terms from the database GOA using @database_go_regex@, e.g. @database_go_regex: ^UniProtKB\\s+(?<shortAccession>\\S+)\\s+\\S+\\s+(?<goTerm>GO:\\d{7})@
